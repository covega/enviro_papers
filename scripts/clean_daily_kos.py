...
INPUT_ROOT = 'data/raw/daily-kos/'
OUTPUT_ROOT = 'data/cleaned/daily-kos/'
...
def clean_daily_kos():
    for xlsx_path, xlsx_cols, output_format in CORRESPONDENCES:
        input_path = os.path.join(os.getcwd(), INPUT_ROOT, xlsx_path)
        output_path = os.path.join(os.getcwd(), OUTPUT_ROOT, output_format)
        df = pd.read_excel(
            input_path,
            sheet_name=None,
            skiprows=1,
